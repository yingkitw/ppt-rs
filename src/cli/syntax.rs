...
pub fn highlight_code(code: &str, language: &str) -> Vec<Vec<HighlightedSegment>> {
    ...
    for line in code.lines() {
        ...
        let segments: Vec<HighlightedSegment> = ranges
            .iter()
...
mod tests {
    ...
    fn test_highlight_colors_not_black() {
        ...
        assert!(
            has_non_black,
            "Syntax highlighting should produce non-black colors"
        );
    }
    ...
    fn test_xml_has_solarized_colors() {
        ...
        let has_color = xml.contains("859900")
            || xml.contains("268BD2")
            || xml.contains("2AA198")
            || xml.contains("B58900")
            || xml.contains("CB4B16")
        ...
        assert!(
            has_color,
            "XML should contain Solarized theme colors, got: {}",
            xml
        );
    }
}
